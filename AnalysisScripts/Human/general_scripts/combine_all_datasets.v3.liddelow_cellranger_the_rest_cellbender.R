...
liddelow <- readRDS(paste(path, "DS000016463_Liddelow_Cellranger.scbasic_out.only_astrocytes.rds", sep=""))
...
for (i in 1:length(all.inputs)) {colnames(all.inputs[[i]]) <- paste(all.inputs[[i]][["studybatch"]], colnames(all.inputs[[i]]), sep="_")} 
...
out <- multiBatchNorm(sce_filtered.more_than_20_cells, normalize.all = TRUE, batch = sce_filtered.more_than_20_cells$studybatch, BPPARAM=MulticoreParam(workers = 28))

print("Saving combine dataset file")
saveRDS(out, "/gstore/data/omni/neuroscience/AD/Astrocytes_AD_MS_PD_meta/AD_MS_PD_cellbender_integration/harmony_integration/integration_with_cellbender_counts/AD_MS_PD_astrocytes.cellbender_counts.20cell_filter.with_logcounts.v3.liddelow_cellranger_rest_cellbender.rds")
print("Done!")
