...
get_umap_plot <- function(scvi_coords_final,cluster) {
  ...
  p <- ggplot(data=scvi_coords_final, aes(x=UMAP1, y=UMAP2)) +
    ...
    geom_point(data = scvi_coords_final %>% dplyr::filter(clusters == cluster), color="dodgerblue2", size=0.5, alpha=0.5)+
    theme_classic() + ggtitle(cluster) + theme(plot.title = element_text(hjust = 0.5)) +
...
get_scvi_coords <- function(se) {
  ...
  results <- fastDummies::dummy_cols(se$clusters)
...
clusters <- unique(out$clusters)

for (i in 1:length(clusters)) {
  p <- get_umap_plot(scvi_coords,clusters[i])
  list_of_plots[[i]] <- p
}


png("/gne/web/dev/apache/htdocs/people/novikovg/Astrocytes_meta/Harmony_AD_MS_PD_integration_2024/AD_PD_MS_studies.clusters_res3.png",width = 4000, height = 3000)
do.call("grid.arrange", c(list_of_plots, ncol=12))
dev.off()
