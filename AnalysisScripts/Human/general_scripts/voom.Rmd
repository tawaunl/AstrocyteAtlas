...
gp.sa.diff::runVoom(se = se_donorunique_agg, groups = "diagnosis_harmonized", 
    comparisons = list(c("AD", "Control")), subset.factor = "studybatch", 
...
# Difference between `AD` vs `Control`
con <- gp.sa.deux::formatComparison(c("diagnosis_harmonizedAD", "diagnosis_harmonizedControl"), design=design)
...
```{r}
all.results[["Difference between `AD` vs `Control`"]] <- DGEStatFrame(
    res, design=design, contrast=con,
    description="Difference between `AD` vs `Control`", 
