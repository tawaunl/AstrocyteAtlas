...
for (k in k_values) {
  rda_fname <- file.path(data.dir,"results",paste0("GoM_Human_k",k,"_results.rda"))
  load(rda_fname)
  if (file.exists(rda_fname)) {
    ...
    annotation <- data.frame(
      ...
      tissue_label = data$ClusterNames)
...
counts <- counts[which(rownames(counts) %in% c(protein_coding$symbol,"NEAT1")),]
