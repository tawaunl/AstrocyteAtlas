...
k_values <- c(4,6, 8, 10, 11, 12, 14,16,18)
...
for (k in k_values) {
  filename <- file.path(out.dir,"results",paste0("fasttopicsHuman_k",k,"_results.rds"))
...
print(loglik_plot)
ggsave("log_likelihood_vs_k_human.png", plot = loglik_plot, width = 8, height = 6)
...
bic_plot <- ggplot(results_df, aes(x = k, y = BIC, color = model)) +
  ...
  scale_x_continuous(breaks = k_values)
print(bic_plot)
ggsave("bic_vs_k.png", plot = bic_plot, width = 8, height = 6)
