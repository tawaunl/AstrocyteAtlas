...
# The matrix should be a sparse count matrix
...
feat <- as.data.frame(genomitory::getFeatures("GMTY17:GRCh38/GRCh38.IGIS4.0.genes.rds@REVISION-3")) # get gene names
library(dplyr)
protein_coding <- feat %>% filter(type =="protein_coding")

filt.counts <- counts[which(rownames(counts) %in% c(protein_coding$symbol,"NEAT1")),]
...
FitGoM(as.matrix(t_deep(filt.counts)), K = k, tol = 0.1,
       path_rda = file.path(results.dir,paste0("GoM_Human_k", k, "_results.rda")))
