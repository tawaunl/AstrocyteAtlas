...
scSigSearch <- function(inpConf, inpMeta, inp, inpGrp, inpPlt,
    ...
    h5file$close_all()
    valid_exprs <- matrix(signature_expr[, colSums(is.na(signature_expr)) == 0])
    if (ncol(valid_exprs) > ){
        ggData$val <- rowMeans(valid_exprs, na.rm = TRUE)
    } else if(ncol(valid_exprs)==1){
        ggData$val <- valid_exprs
    }
    # Handle subsetting
    bgCells = FALSE
    if(length(inpsub2) != 0 & length(inpsub2) != nlevels(ggData$sub)) {
