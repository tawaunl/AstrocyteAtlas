...
runVoom(se = summed, groups = "Disease", comparisons = list(c("AD", 
    "AD_control")), subset.factor = "StudyName", subset.levels = c(study), 
    commit = "never")
```
...
```{r}
subset.factor <- "StudyName"
subset.levels <- "Colonna 5XFAD"
...
```{r}
design <- gp.sa.deux::model.matrix2(data=SummarizedExperiment::colData(se), groups="Disease")
colnames(design)
```
...
# Difference between `AD` vs `AD_control`
con <- gp.sa.deux::formatComparison(c("DiseaseAD", "DiseaseAD_control"), design=design)
...
```{r}
all.results[["Difference between `AD` vs `AD_control`"]] <- DGEStatFrame(
    res, design=design, contrast=con,
    description="Difference between `AD` vs `AD_control`", 
